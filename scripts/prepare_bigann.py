...
def brute_force_gt(base_path: str, queries: np.ndarray, k: int,
                   chunk: int = 25_000, qblock: int = 2_000) -> np.ndarray:
    """Exact top-k by L2, blocked over BOTH queries and base.

    Memory is the binding constraint, not speed: the score matrix is
    (qblock x chunk) float32 and np.argpartition returns an int64 array of the
    same shape, so peak ~ (4+8) * qblock * chunk bytes plus two in-place
    temporaries. At the defaults that is well under 1 GB; the earlier
    single-block version peaked near 40 GB and drove WSL into swap.

    Ranking uses ||b||^2 - 2 q.b (||q||^2 is constant per query and does not
    affect the order), accumulated in place to avoid full-size temporaries.
    """
    dim = queries.shape[1]
    nq = queries.shape[0]
    rec = 4 + dim * 4                      # fvecs record: int32 dim + float32 vector
    total = os.path.getsize(base_path) // rec
    kk = min(k, total)
    out = np.zeros((nq, kk), dtype=np.int64)

    for qs in range(0, nq, qblock):
        q = np.ascontiguousarray(queries[qs:qs + qblock], dtype=np.float32)
        bd = bi = None
        seen = 0
        with open(base_path, "rb") as f:   # one pass per query block
            while seen < total:
                take = min(chunk, total - seen)
                raw = np.frombuffer(f.read(take * rec), dtype=np.float32)
                if raw.size == 0:
                    break
                blk = np.ascontiguousarray(raw.reshape(-1, dim + 1)[:, 1:])
                d = q @ blk.T              # (qb, take) float32
                d *= -2.0                  # in place - no new array
                d += (blk * blk).sum(1)[None, :]
                take_k = min(kk, d.shape[1])
                idx = np.argpartition(d, take_k - 1, axis=1)[:, :take_k]
                ...
                if bd is None:
                    ...
                    bd = np.hstack([bd, dv]); bi = np.hstack([bi, idx])
                    if bd.shape[1] > kk:
                        keep = np.argpartition(bd, kk - 1, axis=1)[:, :kk]
                        bd = np.take_along_axis(bd, keep, axis=1)
                        bi = np.take_along_axis(bi, keep, axis=1)
                seen += blk.shape[0]
                print(f"    brute force q[{qs}:{qs + q.shape[0]}] {seen}/{total}", end="\r")
        order = np.argsort(bd, axis=1)
        out[qs:qs + q.shape[0]] = np.take_along_axis(bi, order, axis=1)[:, :kk]
    print()
    return out
...
def main():
    ...
    ap.add_argument("--chunk", type=int, default=25_000,
                    help="base vectors per block in brute force (memory knob)")
    ap.add_argument("--qblock", type=int, default=2_000,
                    help="queries per block in brute force (memory knob)")
    a = ap.parse_args()
    ...
    for n in a.scales:
        ...
        if os.path.exists(official):
            ...
            gt = brute_force_gt(bpath, q, a.k, chunk=a.chunk, qblock=a.qblock)
